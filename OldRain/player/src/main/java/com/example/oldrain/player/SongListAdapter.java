        ...
        holder.favor_tag.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dataBase.updataData(lovetag.equals("0")?"1":"0", "lovetag",
                        item_contents.get(position).get("path").toString(), "LOCAL");
                if(lovetag.equals("1")){
                    dataBase.deleteData(item_contents.get(position).get("name").toString(),
                            item_contents.get(position).get("path").toString(), "MYLOVE");
                    if(ToolClass.tag.equals("511")){
                           item_contents.remove(position);
                    }
                }else{
                    dataBase.saveData(song, "MYLOVE");
                }
                notifyDataSetChanged();
                ToolClass.show("favor", contexts);
            }
        });
